...
<head>
    ...
    <title>Computer configuration - New domain computer</title>
    <style>
        form select {
            width: 100%;
            padding: 10px;
...
<body>
    <div class="contenedor-form">
        <div class="contenedor-form-botones-guia">
            ...
            <a href="../paginas/formnewdomaincomputer.php">Restablecer</a>
        </div>
        <h1>Nuevo equipo en el dominio</h1>

        <form action="" method="POST">
            <label for="numcomputers">¿Cuántos equipos desea crear en su dominio? (1 al 4)</label>
        ...
        <?php
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["numcomputers"])) {
                ...
                if ($numComputers < 1 || $numComputers > 4) {
                    ...
                    echo "<form action='scriptnewdomaincomputer.php' method='POST'>";
                    echo "<input type='hidden' name='numcomputers' value='$numComputers'>";
                        for ($i = 1; $i <= $numComputers; $i++) {
                            ...
                            echo "<label>¿En qué unidad organizativa lo desea crear?</label>";
                            echo "<input type='text' name='ou$i' placeholder='Equipos ó Sede Zaragoza/Equipos/ST' required>";
                            echo "<label>Nombre de equipo:</label>";
                            echo "<input type='text' name='namecomputer$i' placeholder='SOLPO001I039' required>";
                            echo "<label>Descripción:</label>";
                            echo "<input type='text' name='descriptioncomputer$i' placeholder='Alex Diez Riabets - HP 340s' required>";
                        }
